...
```
If a user tries to submit an input that is less than 1 a warning will appear: prompt on a number field for user to input a value greater than or equal to 1 A similar message will appear if a user tries to input a number greater than 4.
...
```
With the pattern in place, users can’t submit the `<form>` with a number that doesn’t follow the regex. When they try, they’ll see a validation message like so: `message prompting user to follow the requested format`

If you want to find out more about Regex, read more at MDN’s regex article. To see it in practice, let’s use the pattern attribute in our HTML!

#### Validation Review

Awesome job adding client-side validation to `<form>`s! Let’s quickly recap:
